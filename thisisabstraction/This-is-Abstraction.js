/* ===============================================
   ABSTRACTION 2019-2021: ENHANCED PROFESSIONAL GALLERY
   ============================================== */
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🖼️ Annie Rios Gallery - Initializing Abstraction 2019-2021');
    ...
    if (!window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        ...
        const animatedElements = document.querySelectorAll('.artwork-item, .hero-background');
    ...
    enquiryButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            const artworkDetails = {
                ...
                year: '2023',
                medium: 'Acrylic on canvas, oak wood frame',
...
function initMobileAnimations(galleryItems) {
    ...
    const mobileImages = document.querySelectorAll('.artwork-image, .hero-background');
...
function initArtworkModal() {
    ...
    function openModal(index) {
        ...
        // UPDATED: Handle detail images using the global artworkData
        const detailContainer = document.getElementById('detail-images-container');
        if (detailContainer && window.artworkData && window.artworkData[artworkId]) {
            const artworkDetails = window.artworkData[artworkId];
            
            // Clear existing detail images except the main one
            const detailGrid = detailContainer.querySelector('.detail-images-grid');
            const mainDetailImage = detailGrid.querySelector('#main-detail-image');
            detailGrid.innerHTML = '';
            
            // Re-add main detail image
            mainDetailImage.src = imgSrc;
            mainDetailImage.setAttribute('data-full-image', imgSrc);
            detailGrid.appendChild(mainDetailImage);
            
            // Add detail images from artworkData
            if (artworkDetails.detailImages && artworkDetails.detailImages.length > 1) {
                artworkDetails.detailImages.forEach((detailSrc, detailIndex) => {
                    if (detailIndex === 0) return; // Skip first as it's the main image
                    
                    const detailImg = document.createElement('img');
                    detailImg.className = 'detail-image';
                    detailImg.src = detailSrc;
                    detailImg.alt = `${titleText} detail ${detailIndex}`;
                    detailImg.setAttribute('data-full-image', detailSrc);
                    
                    detailImg.addEventListener('click', function() {
                        modalImage.src = this.getAttribute('data-full-image');
                        detailGrid.querySelectorAll('.detail-image').forEach(img => img.classList.remove('active'));
                        this.classList.add('active');
                        resetImageZoom();
                    });
                    
                    detailGrid.appendChild(detailImg);
                });
                
                // Show detail container and set first image as active
...
function initEnhancedImageZoom() {
    ...
    modalImage.addEventListener('click', function(e) {
        // Check if detail container is visible
        const detailContainer = document.getElementById('detail-images-container');
        const isDetailVisible = detailContainer && detailContainer.style.display !== 'none';
...
function initInquiryModal() {
    ...
    window.showInquiryModal = function(artworkDetails) {
        ...
        if (previewYear) previewYear.textContent = artworkDetails.year || '2023';
        if (previewMedium) previewMedium.textContent = artworkDetails.medium || 'Acrylic on canvas, oak wood frame';
...
function initializeLanguageSwitcher() {
    ...
    const translations = {
        en: {
            ...
            // Artworks
            artwork_de_donde_venimos: "De Donde Venimos",
            artwork_gold_mine: "Gold Mine",
            artwork_sailing_spirit: "Sailing Spirit",
            artwork_warm_heart: "Warm Heart",
            artwork_details_medium: "Acrylic on canvas, oak wood frame",
            ...
            inquiry_privacy: "In order to respond to your enquiry, we will process the personal data you have supplied in accordance with our privacy policy."
        },
        ...
        es: {
            ...
            // Artworks
            artwork_de_donde_venimos: "De Donde Venimos",
            artwork_gold_mine: "Mina de Oro",
            artwork_sailing_spirit: "Espíritu Navegante",
            artwork_warm_heart: "Corazón Cálido",
            artwork_details_medium: "Acrílico sobre lienzo, marco de roble",
            ...
            inquiry_privacy: "Para responder a tu consulta, procesaremos los datos personales que has proporcionado de acuerdo con nuestra política de privacidad."
        }
    };
...
function initEnhancedLoadingStates() {
    const images = document.querySelectorAll('.artwork-image, .hero-background');
...
function initEnhancedAnalytics() {
    ...
    window.addEventListener('load', () => {
        ...
        console.log(`🖼️ Abstraction 2019-2021 Gallery loaded in ${loadTime.toFixed(2)}ms`);
    });
...
console.log('✨ Annie Rios - Abstraction 2019-2021 Gallery Loaded');