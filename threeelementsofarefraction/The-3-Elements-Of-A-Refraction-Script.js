/* ===============================================
   THE 3 ELEMENTS OF A REFRACTION JS - ENHANCED PROFESSIONAL GALLERY
   ============================================== */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Remove discontinued artworks
    removeDiscontinuedArtworks();
    injectNewArtworks();
...
function removeDiscontinuedArtworks() {
    const artworks = document.querySelectorAll('.artwork-item');
    artworks.forEach(artwork => {
        const title = artwork.querySelector('.artwork-title');
        if (title && title.textContent.trim().toLowerCase() === 'cost of love') {
            artwork.remove();
        }
    });
}

function injectNewArtworks() {
    const grid = document.querySelector('.collection-grid');
    if (!grid) return;

    const newArtwork = document.createElement('div');
    newArtwork.className = 'artwork-item';
    newArtwork.setAttribute('data-artwork-id', 'what-remains-below');
    
    newArtwork.innerHTML = `
        <div class="artwork-image-container">
            <img src="../images/IMG_4741.JPG" alt="What Remains Below" class="artwork-image" loading="lazy">
        </div>
        <div class="artwork-info">
            <h3 class="artwork-title">What Remains Below</h3>
            <div class="artwork-details">
                <div>Acrylics on canvas</div>
                <div>194 cm (alto) × 155 cm (ancho)</div>
                <div>Diptych, 2 panel artwork</div>
            </div>
            <a href="#" class="enquire-button" style="color: #000; text-decoration: none;">Enquire</a>
        </div>
    `;

    // Insert as the first item (newest release)
    grid.prepend(newArtwork);
}
...
// =============================================
// ENHANCED DETAIL IMAGES FUNCTIONALITY
// =============================================

function initDetailImages(mainImageSrc) {
  const detailImages = document.querySelectorAll('.detail-image');
  const mainModalImage = document.getElementById('modal-image');
  const mainDetailImage = document.getElementById('main-detail-image');
  
  if (!detailImages.length || !mainModalImage || !mainDetailImage) return;
...
function initArtworkModal() {
    ...
    let originalDetailContent = '';
    if (detailContainer) {
        originalDetailContent = detailContainer.innerHTML;
    }
    ...
    function openModal(index) {
        ...
        if (detailContainer) {
            if (artworkId === 'what-remains-below') {
                detailContainer.style.display = 'block';
                // Inject new detail images
                detailContainer.innerHTML = `
                    <div class="detail-scroll-wrapper" style="display: flex; gap: 15px; overflow-x: auto; padding: 20px 0; justify-content: center; flex-wrap: wrap;">
                        <img src="../images/IMG_4741.JPG" class="detail-image active" id="main-detail-image" data-full-image="../images/IMG_4741.JPG" alt="Main View" style="height: 80px; width: auto; cursor: pointer; object-fit: cover;">
                        <img src="../images/IMG_4691.JPG" class="detail-image" data-full-image="../images/IMG_4691.JPG" alt="Detail 1" style="height: 80px; width: auto; cursor: pointer; object-fit: cover;">
                        <img src="../images/IMG_4727.JPG" class="detail-image" data-full-image="../images/IMG_4727.JPG" alt="Detail 2" style="height: 80px; width: auto; cursor: pointer; object-fit: cover;">
                        <img src="../images/IMG_4734.JPG" class="detail-image" data-full-image="../images/IMG_4734.JPG" alt="Detail 3" style="height: 80px; width: auto; cursor: pointer; object-fit: cover;">
                        <img src="../images/IMG_4751.JPG" class="detail-image" data-full-image="../images/IMG_4751.JPG" alt="Detail 4" style="height: 80px; width: auto; cursor: pointer; object-fit: cover;">
                    </div>
                `;
                setTimeout(() => initDetailImages(imgSrc), 100);
            } else if (artworkId === 'be-the-light') {
                // Restore original content if needed
                if (originalDetailContent && detailContainer.innerHTML !== originalDetailContent) {
                    detailContainer.innerHTML = originalDetailContent;
                }
                detailContainer.style.display = 'block';
                
                // Reset detail images active state
                const detailImages = document.querySelectorAll('.detail-image');
                detailImages.forEach(img => img.classList.remove('active'));
...
function initEnhancedImageZoom() {
    ...
    modalImage.addEventListener('click', function(e) {
        // Don't zoom if we're on a detail image for BE THE LIGHT
        const detailContainer = document.getElementById('detail-images-container');
...
// KEEP YOUR EXISTING INQUIRY MODAL - NO CHANGES NEEDED
...
// KEEP YOUR EXISTING LANGUAGE SWITCHER - NO CHANGES
function initializeLanguageSwitcher() {
    ...
    const translations = {
        en: {
            ...
            "hero_title": "The Three Elements",
            "hero_subtitle": "of a Refraction Collection",
            "hero_year": "2024-2025",
            "comp_of_refraction_1": "Composition of a Refraction 1",
            "comp_of_refraction_2": "Composition of a Refraction 2",
            "screen_3_title": "Rhythms and Refractions",
            "screen_5_title": "Rhythms and Refractions 2",
            "screen_6_title": "Point of Incidence",
            "the_inbetween_space": "The InBetween Space",
            "acrylic_mixed_canvas": "Acrylic and Mixed Media on Canvas",
            "acrylic_mixed_canvas_lc": "Acrylic and mixed media on canvas",
            "screen_1_description": "134 x 194 cm",
            "screen_2_description": "134 x 187 cm",
            "screen_3_description": "100 x 154 cm",
            "screen_5_description": "100 x 154 cm",
            "screen_6_description": "97 x 114 cm",
            "dimension_92_114": "92 x 114 cm",
            "collection_learn_more": "Learn more about this collection",
            "collection_desc_title": "THREE ELEMENTS OF A REFRACTION",
            "collection_desc_subtitle": "Annie Ríos, 2025",
            "collection_desc_p1": "Three Elements of A Refraction examines light's encounter with matter as both optical fact and perceptual challenge. Ríos isolates the three-stage process of refraction—incident ray, point of incidence, refracted ray—using it as structural framework for a body of work that questions how contemporary acceleration has diminished our capacity for sustained visual attention.",
            ...
            "collection_desc_p3": "Working from Panama City, Ríos describes these works as \"intimate spaces for absolute personal introspection\"—paintings that demand optical commitment rather than passive consumption. The exhibition proposes that sustained attention to physical phenomena constitutes a form of resistance to distraction. Refraction functions here as both subject and method: a reproducible optical event made visible through paint's material properties.",
            "collection_desc_p4": "The body of work positions abstraction not as departure from observable reality but as means of documenting it. These are paintings of something that exists—light bending through matter—insisting that deliberate looking might recalibrate habitual perception.",
        ...
        es: {
            ...
            "hero_title": "Los Tres Elementos",
            "hero_subtitle": "de una Colección de Refracción",
            "hero_year": "2024-2025",
            "comp_of_refraction_1": "Composición de una Refracción 1",
            "comp_of_refraction_2": "Composición de una Refracción 2",
            "screen_3_title": "Ritmos y Refracciones",
            "screen_5_title": "Ritmos y Refracciones 2",
            "screen_6_title": "Punto de Incidencia",
            "the_inbetween_space": "El Espacio Intermedio",
            "acrylic_mixed_canvas": "Acrílico y Medios Mixtos sobre Lienzo",
            "acrylic_mixed_canvas_lc": "acrílico y medios mixtos sobre lienzo",
            "screen_1_description": "134 x 194 cm",
            "screen_2_description": "134 x 187 cm",
            "screen_3_description": "100 x 154 cm",
            "screen_5_description": "100 x 154 cm",
            "screen_6_description": "97 x 114 cm",
            "dimension_92_114": "92 x 114 cm",
            "collection_learn_more": "Conoce más sobre esta colección",
            "collection_desc_title": "LOS TRES ELEMENTOS DE UNA REFRACCIÓN",
            "collection_desc_subtitle": "Annie Ríos, 2025",
            "collection_desc_p1": "Los Tres Elementos de una Refracción examina el encuentro de la luz con la materia como hecho óptico y desafío perceptual. Ríos aísla el proceso de tres etapas de la refracción—rayo incidente, punto de incidencia, rayo refractado—utilizándolo como marco estructural para un cuerpo de obra que cuestiona cómo la aceleración contemporánea ha disminuido nuestra capacidad de atención visual sostenida.",
            ...
            "collection_desc_p3": "Trabajando desde Ciudad de Panamá, Ríos describe estas obras como \"espacios íntimos para la introspección personal absoluta\"—pinturas que exigen compromiso óptico en lugar de consumo pasivo.",
            "collection_desc_p4": "El cuerpo de obra posiciona la abstracción no como alejamiento de la realidad observable sino como medio para documentarla. Estas son pinturas de algo que existe—la luz doblándose a través de la materia—insistiendo en que mirar deliberadamente podría recalibrar la percepción habitual.",
...
// KEEP YOUR EXISTING SHARE BUTTON - NO CHANGES
